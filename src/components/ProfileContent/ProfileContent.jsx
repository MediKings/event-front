...
export const ProfileContent = ({user}) => {
  ...
  useEffect(() => {
    const fetchMyEvents = async () => {
      const resp = await axios.get(`https://event-back-ow5h.onrender.com/api/myEvents/${user.id}`)
  ...
  useEffect(() => {
    const fetchMyFollowedEvents = async () => {
      const resp = await axios.get(`https://event-back-ow5h.onrender.com/api/getFollowedEvent/${user.id}`)
                ...
                {myFollowedEvents.map((e) => {
                    return (
                        <div className="col-md-6 col-lg-4 p-2" key={e.id}>
                            <div className="event">
                                <Link to={`/event/${e.event.id}`}><img src={`https://event-back-ow5h.onrender.com/images/${e.event.image}`} alt="" /></Link>
